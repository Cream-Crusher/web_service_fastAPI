...
async def try_create_quiz(session: AsyncSession, question: dict) -> True | False:
...
async def try_get_prev_quiz(session: AsyncSession) -> Quiz:
    result = await session.execute(
        select(Quiz)
        .options(selectinload(Quiz.category))
        .order_by(desc(Quiz.parsed_at))
    )
